(function ()
{
    ...
    angular
        .module('app.stations.list')
        .controller('StationsListController',StationsListController);

    /** @ngInject */
    function StationsListController($scope,$state, api,$mdDialog,$rootScope,standardizer)
    {
        var vm = this;
        // Data
        $scope.head = {
            ico:"icon-account-box",
            title:"Liste stations d'agréage'"
        };
        ...
        $scope.loadPage = function() {
            api.stations.getAll.get({ pid:$scope.paginationOptions.pageNumber,nbp:$scope.paginationOptions.pageSize },
                // Success
                function (response)
                {
        ...
        $scope.add = function() {
            $state.go("app.stations_edit", { id:-1 });
        };
        $scope.edit = function(id) {
            $state.go("app.stations_edit", { id:id });
        };
        $scope.remove = function(id,ev) {
            ...
            $mdDialog.show(confirm).then(function() {
                $rootScope.loadingProgress = true;
                api.stations.delete.delete({ id:id } ,
                // Success
                function (response)
                {
                    $scope.loadPage();
