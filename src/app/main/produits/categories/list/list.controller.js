(function ()
{
    ...
    angular
        .module('app.produits.categories.list')
        .controller('ProduitsCategoriesListController',ProduitsCategoriesListController);

    /** @ngInject */
    function ProduitsCategoriesListController($scope,$state, api,$mdDialog,$rootScope,standardizer)
    {
        var vm = this;
        // Data
        $scope.head = {
            ico:"icon-account-box",
            title:"Liste catégories produits"
        };
        ...
        $scope.loadPage = function() {
            api.productsCategories.getAll.get({ pid:$scope.paginationOptions.pageNumber,nbp:$scope.paginationOptions.pageSize },
                // Success
                function (response)
                {
                    $scope.maxSize = 5;
        ...
        $scope.add = function() {
            $state.go("app.produits_categories_edit", { id:-1 });
        };
        $scope.edit = function(id) {
            $state.go("app.produits_categories_edit", { id:id });
        };
